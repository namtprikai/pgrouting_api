...
def multimodal_route(payload: MultimodalBody):
    ...
    if start_node_component == end_node_component:
        ...
        # セグメント計算
        seg_truck_OD = route_truck_mm(O["lon"], O["lat"], D["lon"], D["lat"], toll_per_km)

        seg_truck_O_StO = route_truck_mm(O["lon"], O["lat"], stO["slon"], stO["slat"], toll_per_km) if stO else None
        seg_train_StO_StD = route_train(stO["id"], stD["id"], wait_train) if (stO and stD) else None
        seg_truck_StD_D = route_truck_mm(stD["slon"], stD["slat"], D["lon"], D["lat"], toll_per_km) if stD else None

        seg_truck_O_PtO = route_truck_mm(O["lon"], O["lat"], ptO["plon"], ptO["plat"], toll_per_km) if ptO else None
        seg_ship_PtO_PtD = route_ship_direct(ptO["id"], ptD["id"], ship_speed, wait_ship) if (ptO and ptD) else None
        seg_truck_PtD_D = route_truck_mm(ptD["plon"], ptD["plat"], D["lon"], D["lat"], toll_per_km) if ptD else None
        ...
        if stD and ptO and seg_truck_O_StO and seg_train_StO_StD and seg_ship_PtO_PtD and seg_truck_PtD_D:
            seg_truck_StD_PtO = route_truck_mm(stD["slon"], stD["slat"], ptO["plon"], ptO["plat"], toll_per_km)
            if seg_truck_StD_PtO:
                patterns.append({
                    "key": "TRUCK_TRAIN_SHIP",
        ...
        if ptD and stO and seg_truck_O_PtO and seg_ship_PtO_PtD and seg_train_StO_StD and seg_truck_StD_D:
            seg_truck_PtD_StO = route_truck_mm(ptD["plon"], ptD["plat"], stO["slon"], stO["slat"], toll_per_km)
            if seg_truck_PtD_StO:
                patterns.append({
                    "key": "TRUCK_SHIP_TRAIN",
        ...
        if stO and stD and ptO and ptD:
            seg_truck_O_StO2 = seg_truck_O_StO or route_truck_mm(O["lon"], O["lat"], stO["slon"], stO["slat"], toll_per_km)
            seg_train_StO_StD2 = seg_train_StO_StD or route_train(stO["id"], stD["id"], wait_train)
            seg_truck_StD_PtO2 = route_truck_mm(stD["slon"], stD["slat"], ptO["plon"], ptO["plat"], toll_per_km)
            seg_ship_PtO_PtD2  = seg_ship_PtO_PtD or route_ship_direct(ptO["id"], ptD["id"], ship_speed, wait_ship)
            seg_truck_PtD_StO2 = route_truck_mm(ptD["plon"], ptD["plat"], stO["slon"], stO["slat"], toll_per_km)
            seg_train_StO_StD3 = route_train(stO["id"], stD["id"], wait_train)
            seg_truck_StD_D2   = seg_truck_StD_D or route_truck_mm(stD["slon"], stD["slat"], D["lon"], D["lat"], toll_per_km)
        ...
        for pat in patterns:
            ...
            co2_g_total = 0.0
            for s in pat["segs"]:
                ef = ef_truck if s["mode"] == "truck" else (ef_train if s["mode"] == "train" else ef_ship)
                co2_g_total += co2_g(s["distance_km"], ef, ton)
        ...
        bridge_node = find_bridge_nodes(start_node, end_node)
        print(bridge_node, 'bridge_node')
        raise HTTPException(status_code=404, detail="No feasible pattern could be constructed (check station/port coverage).")
