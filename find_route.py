...
Usage examples:
  ...
  python find_route.py 35.6762 139.6503 34.6937 135.5023 --output results
    ...
    parser.add_argument(
        "--weight", type=float, default=10.0, help="Cargo weight (tons)"
    )
    parser.add_argument(
        "--data-folder",
    ...
    try:
        ...
        # Display results
        print_route_results(
            results, args.criteria, args.verbose, args.mode, args.show_all
        )
...
def print_route_results(
    results, criteria="fastest", verbose=False, mode="all", show_all=False
):
