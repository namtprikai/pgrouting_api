...
def multimodal_route(payload: MultimodalBody):
    ...
    if payload.mode.upper() in STREET_TYPE:
        # 基本パラメータ
        origin_lat = (
            float(payload.origin_lat) if payload.origin_lat is not None else None
        )
        origin_lon = (
            float(payload.origin_lon) if payload.origin_lon is not None else None
        )
        ...
        mode = (
            ...
            else STREET_TYPE["TRUCK_ONLY"].lower()
        )
        ...
        if show_all:
            ...
            if criteria in optimal_routes:
                ...
                selected_route = (
                    optimal_route_full if optimal_route_full else optimal_route_summary
                )
        ...
        summary = {
            "time": (
                data["total_time_minutes"] / 60
                if "total_time_minutes" in data and data["total_time_minutes"]
                else None
            ),
            "distance": (
                data["total_distance_km"]
                if "total_distance_km" in data and data["total_distance_km"]
                else None
            ),
            "co2": (
                data["co2_emissions_grams"]
                if "co2_emissions_grams" in data and data["co2_emissions_grams"]
                else None
            ),
            "mode": mode,
            "origin_port": (
                data["origin_port"]
                if "origin_port" in data and data["origin_port"]
                else None
            ),
            "dest_port": (
                data["dest_port"] if "dest_port" in data and data["dest_port"] else None
            ),
            "origin_station": (
                data["origin_station"]
                if "origin_station" in data and data["origin_station"]
                else None
            ),
            "dest_station": (
                data["dest_station"]
                if "dest_station" in data and data["dest_station"]
                else None
            ),
            "transfer_port": (
                data["transfer_port"]
                if "transfer_port" in data and data["transfer_port"]
                else None
            ),
            "transfer_station": (
                data["transfer_station"]
                if "transfer_station" in data and data["transfer_station"]
                else None
            ),
        }
    else:
        summary = {"mode": payload.mode, "msg": "Street cann't found"}
    return summary
